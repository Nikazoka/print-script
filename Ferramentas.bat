...
cls
echo Opcao errada! Pressione qualquer tecla para voltar ao menu...
pause >nul
goto menu
...
:att
cls
echo O navegador sera encerrado pra atualizacao!
echo.
...
set "PASTA_DESTINO=%USERPROFILE%\Documents\extensao"
...
:cac
cls
echo Apos a limpeza do cache sera necessario realizar novamente o login nos sistemas.
echo.
set /p r=Deseja continuar? » S/N 
...
:any
cls
echo Sera necessario reiniciar o computador!
set /p r=Deseja continuar? » S/N 
...
set "ARQUIVO_TEMP=%TEMP%\script_novo_linux.tmp"
set "ARQUIVO_FINAL=%TEMP%\script_novo.bat"
...
:: Converte o arquivo do formato Linux (LF) para o formato Windows (CRLF)
more < "%ARQUIVO_TEMP%" > "%ARQUIVO_FINAL%"
...
echo Disponivel entre 7h - 12h e 14h - 17h
